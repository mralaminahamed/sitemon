...
func (d *Database) SaveCheck(record CheckRecord) error {
	...
	// Pass the time.Time directly and store as UTC. go-sqlite3 declares these
	// columns DATETIME and round-trips them to/from time.Time itself.
	_, err := d.db.Exec(query, record.URL, record.Status, record.StatusCode, record.ResponseTime, record.Timestamp.UTC(), record.Error)
	return err
}
...
func (d *Database) SaveRequest(record RequestRecord) error {
	...
	_, err := d.db.Exec(query, record.URL, record.Method, record.Status, record.StatusCode, record.ResponseTime, record.Timestamp.UTC())
	return err
}
...
func (d *Database) GetChecks(limit int, from, to *time.Time) ([]CheckRecord, error) {
	...
	for rows.Next() {
		var r CheckRecord
		if err := rows.Scan(&r.ID, &r.URL, &r.Status, &r.StatusCode, &r.ResponseTime, &r.Timestamp, &r.Error); err != nil {
			return nil, err
		}
...
func (d *Database) GetRequests(limit int, from, to *time.Time) ([]RequestRecord, error) {
	...
	for rows.Next() {
		var r RequestRecord
		if err := rows.Scan(&r.ID, &r.URL, &r.Method, &r.Status, &r.StatusCode, &r.ResponseTime, &r.Timestamp); err != nil {
			return nil, err
		}
		records = append(records, r)
	}
