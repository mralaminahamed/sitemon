...
Reports detailed latency statistics including p50, p90, p95, and p99 percentiles.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// Cancel the run cleanly on Ctrl+C / SIGTERM. limiter.Wait(ctx)
		// unblocks on cancel so the send loop stops promptly.
		...
		// One shared limiter for the whole run. The old code built a fresh
		// limiter every iteration, which reset the token bucket each time and
		// meant the RPS cap was never actually enforced. Ramp-up now adjusts
		// this single limiter's rate via SetLimit.
		limiter := rate.NewLimiter(rate.Limit(rps), rps)

		runTest := func() {
			for i := 0; ; i++ {
				if !isDurationMode && i >= totalRequests {
					break
				}
				if ctx.Err() != nil {
					break
				...
				if rampUp > 0 && time.Now().Before(rampUpEndTime) {
					...
					currentRPS := float64(rps) * float64(elapsedRamp) / float64(rampUpDuration)
					if currentRPS < 1 {
						currentRPS = 1
					}
				...
				g.Go(func() error {
					...
					// Min/max are read-then-write, not atomic on their own, so
					// guard with a mutex. The old compare + atomic.Store raced
					// and lost updates under -race.
					sizeMu.Lock()
					if minResponseSize == -1 || result.ResponseSize < minResponseSize {
						minResponseSize = result.ResponseSize
					}
					if result.ResponseSize > maxResponseSize {
						maxResponseSize = result.ResponseSize
					}
					sizeMu.Unlock()
		...
		// Safe to read directly: all workers have finished (g.Wait returned).
		minRespSize := minResponseSize
