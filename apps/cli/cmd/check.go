...
import (
	...
	"github.com/mralaminahamed/sitemon/packages/shared/validation"
		...
		case <-ticker.C:
			for _, url := range urls {
				...
				if err != nil {
					...
					// Treat a breach of the latency threshold as a WARNING so it
					// drives an alert transition like any other degraded state.
					if maxLatency > 0 && result.ResponseTime > maxLatency && status == "UP" {
						status = "WARNING"
					}
					...
					// Evaluate every tick (including UP) so recovery is detected;
					// Evaluate only fires on an actual transition.
					alerter.Evaluate(url, status, result.StatusCode)
				}
			}
